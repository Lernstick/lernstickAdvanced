...
build_image()
{
	...
	sed -i "s|title-text.*|title-text: \"Lernstick-Prüfungsumgebung Debian 12 (Version KSCS ${TODAY})\"|1" \
	...
	if [ -f ${ISO_FILE} ]
	then
		PREFIX="lernstick_pruefungsumgebung_debian12_kscs${ISO_SUFFIX}_${TODAY}"
		IMAGE="${PREFIX}.iso"
		mv ${ISO_FILE} ${IMAGE}
