...
build_image()
{
	...
	sed -i "s|title-text.*|title-text: \"Lernstick-Prüfungsumgebung Debian 9 (Version ${TODAY})\"|1" \
	...
	lb config \
		...
		--linux-packages linux-image-4.13.0-0.bpo.1 \
	...
	if [ -f ${ISO_FILE} ]
	then
		PREFIX="lernstick_pruefungsumgebung_debian9${ISO_SUFFIX}_${TODAY}"
		IMAGE="${PREFIX}.iso"
		mv ${ISO_FILE} ${IMAGE}
